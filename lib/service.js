...
class ExplorerUI {
    constructor(options) {
        ...
        fs.writeFileSync('www/js/ows-node-config.js', `angular.module('owsExplorerApp').constant('nodeConfig', ${  JSON.stringify(config, null, 2)  });`);

        this.app = express();
    }

    async start() {
        this.indexFile = this.filterIndexHTML(fs.readFileSync('www/index.html', {encoding: 'utf8'}));
        this.app.use(express.static('www', {index: false}));
