...
angular.module('owsExplorerApp.controllers').controller('ScannerController', function($scope, $rootScope, $uibModalInstance, NodeService) {
  ...
  qrcode.callback = function(data) {
    ...
    if (str.indexOf(':') > 0) {
      ...
      var requiredNode = NodeService.getNodeByProtocol(protocol);

      if (requiredNode) {
        NodeService.setNode(requiredNode.info.description.name);
      } else {
        $rootScope.flashMessage = 'Warning: unknown protocol or node not available\'' + protocol + '\'';
        console.log('Warning: unknown protocol or node not available \'' + protocol + '\'');
      }
